...
local Groups = require("mage.groups")
...
function M.extras()
  local Extra = require("mage.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/magi/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/magi/config.lua", "\n(--@class magi%.Config.-\n})")
