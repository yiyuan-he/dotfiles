...
function M.setup()
  ...
  -- mage.setup({ plugins = { all = true } })
  for _, extra in ipairs(names) do
    local info = M.extras[extra]
    local plugin = require("mage.extra." .. extra)
    for style, style_name in pairs(styles) do
      local colors, groups, opts = mage.load({ style = style, plugins = { all = true } })
      local fname = extra
      ...
      colors["_upstream_url"] = "https://github.com/folke/mage.nvim/raw/main/extras/" .. fname
      colors["_style_name"] = "Mage" .. style_name
      colors["_name"] = "mage_" .. style
      colors["_style"] = style
      print("[write] " .. fname)
